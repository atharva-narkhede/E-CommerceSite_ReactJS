...
const Cart = ({ cartItems, fetchCart, handleUpdateCart, handleRemoveFromCart }) => {
    const total = cartItems.reduce((sum, item) => sum + (item.p_cost * item.quantity), 0);
        ...
        <div className="container mt-5">
            ...
            <div className="card mb-4">
                <div className="card-body">
                    <h4 className="card-title text-center">Total: ₹{total.toFixed(2)}</h4>
                </div>
            </div>
            {cartItems.length > 0 ? (
                <div className="row">
                        ...
                        <div className="col-md-4 mb-4" key={index}>
                            <div className="card h-100">
                                <div className="card-body text-center">
                                    ...
                                    />
                                    <h5 className="card-title mt-2">{item.p_name}</h5>
                                    <p className="card-text">Price: ₹{item.p_cost}</p>
